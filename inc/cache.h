...
//            this->roi_stats[metric] = 0;
//            this->total_stats[metric] = 0;
//        }
//        this->cur_stats =nullptr;
//    }
//
//    void prefetcher_operate( uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type) {
//        /* don't do anything if CPU is in warmup phase */
//        if(type==LOAD)
//        {
//            if(this->cur_stats==nullptr)
//                return;
//            auto& stats = *this->cur_stats;
...
//            /* check prefetch hit */
//            if (cache_hit == 1) {
//                int cnt = this->prefetched_blocks.erase(addr);
//                if (cnt == 1) {
//                    (*this->cur_stats)["Prefetch Hits"] += 1;
//                    (*this->cur_stats)["Undecided Prefetches"] -= 1;
//                }
//            }
//
//        }
//    }
//
//    void prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr) {
//        /* don't do anything if CPU is in warmup phase */
//        if(this->cur_stats==nullptr)
//            return;
//        auto& stats = *this->cur_stats;
...
//        int cnt = this->prefetched_blocks.erase(evicted_addr);
//        if (cnt == 1) {
//            (*this->cur_stats)["Non-useful Prefetches"] += 1;
//            (*this->cur_stats)["Undecided Prefetches"] -= 1;
//        }
//        if (prefetch == 1) {
//            stats["Prefetches"] += 1;
//            stats["Undecided Prefetches"] += 1;
//            prefetched_blocks.insert(addr);
//        }
//    }
//
//    void prefetcher_inform_warmup_complete() {
//        this->cur_stats=&this->roi_stats;
//    }
//
//    void prefetcher_inform_roi_complete() {
//        this->total_stats = this->roi_stats; /* copy roi_stats over to total_stats */
//        this->cur_stats = &this->total_stats;
//    }
//
//    void prefetcher_final_stats(uint32_t cpu,string level) {
//        this->print_stats(this->total_stats, "Total", cpu,level);
//    }
//
//    void prefetcher_roi_stats(uint32_t cpu,string level) {
//        this->print_stats(this->roi_stats, "ROI", cpu,level);
//    }
//
//    void prefetcher_cur_stats(uint32_t cpu,string level) {
//        this->print_stats(*this->cur_stats, "Current", cpu,level);
//    }
//
//    void print_stats(unordered_map<string, uint64_t> stats, string name, uint32_t cpu,string level) {
//        for (auto &metric : this->metrics)
//            cout << "* CPU " << cpu << " " << name << " " <<level<<'_'<<metric << ": " << stats[metric] << endl;
//    }
//
//private:
//    vector<string> metrics = {"Accesses", "Misses", "Prefetches", "Prefetch Hits", "Non-useful Prefetches", "Undecided Prefetches"};
//    unordered_map<string, uint64_t> roi_stats, total_stats;
//    unordered_map<string, uint64_t> *cur_stats;
//    unordered_set<uint64_t> prefetched_blocks;
//};
...
//    }
//
//    void prefetcher_operate(uint32_t cpu, uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type) {
//        /* don't do anything if CPU is in warmup phase */
//        if(type==LOAD)
//        {
...
//    }
//
//    void prefetcher_cache_fill(uint32_t cpu, uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr) {
//        /* don't do anything if CPU is in warmup phase */
//        if (!this->cur_stats[cpu])
//            return;
//        auto& stats = *this->cur_stats[cpu];
...
//    LLC_Stats llc_stats;
    ...
    int  check_hit(PACKET *packet),
         ...
         prefetch_line_diff_page(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int prefetch_fill_level, uint32_t prefetch_metadata);
...
//    void l1d_prefetcher_inform_warmup_complete(),
//        l2c_prefetcher_inform_warmup_complete(),
//        llc_prefetcher_inform_warmup_complete(),
//        l1d_prefetcher_inform_roi_complete(),
//        l2c_prefetcher_inform_roi_complete(),
//        llc_prefetcher_inform_roi_complete(uint32_t cpu),
//        l1d_prefetcher_roi_stats(),
//        l2c_prefetcher_roi_stats(),
