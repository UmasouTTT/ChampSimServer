...
map<uint64_t, vector<uint64_t>>valuless_ip_pattern;
ChampSimLog cslog("valuess_pattern.txt");
...
vector<uint64_t> read_important_files(string file_path)
{
    ...
    if (file) // 有该文件
    {
        while (getline(file, line)) // line中不包括每行的换行符
        {
            ...
            valuless_ip_pattern[stoi(line)] = vector<uint64_t>();
        }
    }
    else // 没有该文件
    {
...
void CACHE::l1d_prefetcher_initialize()
{
    ...
    //ip classify
    important_ips = read_important_files("valuless_ips.txt");
    cslog.makeLog(string("finish read valuless ips!"), true);
    cslog.makeLog(string("ips size: " + to_string(important_ips.size())), true);
    for(auto it = important_ips.begin(); it != important_ips.end(); it++){
        cslog.makeLog(string("ip: " + to_string((*it)) + " is valuless!"), true);
    }
...
void CACHE::l1d_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type)
{
    ...
    if(important_ips.end() == find(important_ips.begin(), important_ips.end(), ip)){
        ...
        valuless_ip_pattern[ip].push_back(addr);
    }
...
{
    for (auto ip_itor = valuless_ip_pattern.begin(); ip_itor != valuless_ip_pattern.end() ; ip_itor++) {
